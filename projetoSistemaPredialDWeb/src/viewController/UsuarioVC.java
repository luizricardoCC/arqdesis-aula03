...
public class UsuarioVC extends JDialog implements ActionListener, ItemListener
{
   private JLabel                lTipoUsuario, lNome, lCpf, lDataNascimento, lEndereco, lBairro, lTelefone, lSenha, lEmpresa, lHorarioDeAcesso, lEntre, lE;
   private JTextField            tfNome, tfEndereco, tfBairro;
   private JFormattedTextField   ftfCpf, ftfDataNascimento, ftfTelefone, ftfHorarioDeAcessoInicial, ftfHorarioDeAcessoFinal;
   ...
   private JPanel                pBase, pDados, pDadosComuns, pDadosFuncionario, pTipoUsuario, pNome, pCpf, pDataNascimento, pEndereco, pTelefone, pSenha, pEmpresa, pHorarioDeAcesso, pBotoes;
   ...
   /*
   O parâmetro neste construtor indica qual a forma que será construida a janela deste frame quando este for instanciado
      0                      -> janela para cadastro de funcionário
      1                      -> janela para cadastro de atendente ou síndico
      Qualquer outro inteiro -> janela para consulta de usuários 
   */
   public UsuarioVC(int modo)
   {
      
      
      
      tipoUsuario                         = 0;
      bundle                              = ResourceBundle.getBundle( "properties/idioma", new Locale("pt", "BR") );
      
      String[]          tipoUsuario       = { bundle.getString("tuFuncionario"), bundle.getString("tuAtendente"), bundle.getString("tuSindico") };
      String            mascaraCpf        = "###.###.###-##";
      String            mascaraData       = "####/##/##";
   ...
   public void construirJanelaConsulta()
   {
      ...
      //Desabilita as opções de entrada, só sendo acessível a edição quando o botão bAlterar for clicado
   ...
   {
      if(modo == 0) // modo de cadastro para síndico. Pode cadastrar todos tipos de usuários
      {
         construirPainelPadrao();
         construirPainelFuncionario(); 
      }
      else if(modo == 1) //modo de cadastro para atendente. Pode cadastrar somente atendentes ou funcionários
      {
         construirPainelPadrao();
         construirPainelFuncionario();
   ...
   public void actionPerformed(ActionEvent actionE)
   {
      if(actionE.getSource() == bCadastrar)
      {
         ...
         if( validarCPF( cpf ) == false ) return;
         ...
         {
            int resp = JOptionPane.showConfirmDialog(null, bundle.getString("msg2"), bundle.getString("msgTitulo2"), JOptionPane.YES_NO_OPTION);
         ...
         //Preparando informações para o cadastro no banco
         ...
         if( cbTipoUsuario.getSelectedIndex() == 0){ 
        	 usuarioEmpresa = cbEmpresa.getSelectedItem().toString();
         }
         ...
         Usuario uNovo = new Usuario( cbTipoUsuario.getSelectedIndex(),
                 ...
                 s, 
                 usuarioEmpresa, 
                 ftfHorarioDeAcessoInicial.getText(), 
                 ftfHorarioDeAcessoFinal.getText(), 
                 cpf, 
                 chbPermissaoFuncionario.isSelected() 
                );
                                          
         boolean resultado = usCadastro.cadastrarUsuario( uNovo );
         if( resultado )
         {
      ...
      if(actionE.getSource() == bConsultar)
      {
         ...
         /* A varíavel local tipoUsuario referencia um tipo de usuário a um número inteiro.
            ...
            O tipo de usuario deve ser cadastrado no banco como um Inteiro e NUNCA deve ser nulo.
            Quando a classe model não tiver encontrado nenhum Usuário com o CPF passado, ela deve retornar -1 para
         ...
         */
         ...
         {
            construirFrame(0); //a construção do frame vai se alterar de acordo com o tipo de usuário retornado
            System.out.println( "Empresa em UVC" + uTO.getEmpresa());
            int tmp = 0;
            for( int i = 0; i < listaEmpresas.length; i++){
            	if(listaEmpresas[i] == uTO.getEmpresa())
            		tmp = i;
            }
            cbEmpresa.setSelectedIndex( tmp );
            ftfHorarioDeAcessoInicial.setText( uTO.getHorEntrada() );
            ftfHorarioDeAcessoFinal.setText( uTO.getHorSaida() );
            chbPermissaoFuncionario.setSelected( uTO.getPermissaoAC() );
         }
         else
         {
            construirFrame(1);
         ...
         //Carregando as informações do usuário consultado, quando encontrado alguém
         lastCPF = cpf;
         tfNome.setText( uTO.getNome() );
         ftfDataNascimento.setText( uTO.getDataNasc().replace("-", "") );
         tfEndereco.setText( uTO.getEndereco() );
         tfBairro.setText( uTO.getBairro() );
         ftfTelefone.setText( String.valueOf( uTO.getTelefone() ) );
         pfSenha.setText( uTO.getSenha() );
         //
      ...
      if(actionE.getSource() == bSalvar)// só aparece depois de apertado o botão bAlterar
      {
         //Validando algumas informações antes de prosseguir para o cadastro
         String   d = ftfDataNascimento.getText();
         if( validarData(d ) == false)
         ...
         //Preparando informações para alteração o banco
         String usuarioEmpresa = "";
         String horEntrada = "";
         String horSaida = "";
         Long usuarioCpf = (long) 0;
         boolean permissaoAC = false;
         
         if(tipoUsuario == 0)
         { 
            usuarioEmpresa = cbEmpresa.getSelectedItem().toString();
            horEntrada = ftfHorarioDeAcessoInicial.getText();
            horSaida = ftfHorarioDeAcessoFinal.getText();
            usuarioCpf = Long.parseLong( ftfCpf.getText().replace(".", "").replace("-", "") );
            permissaoAC = chbPermissaoFuncionario.isSelected();
         }
         //
         ...
         boolean resultado = usAlterar.alterarUsuario( this.lastCPF, uNovo ); 
      ...
      {
         int resp = JOptionPane.showConfirmDialog(null, bundle.getString("msg9"), bundle.getString("msgTitulo9"), JOptionPane.YES_NO_OPTION);
         ...
         {  
            ...
            String   msg      = "";
            int      n        = 0; 
            if ( usuarioService.excluirUsuario( cpf ) )
            {
   ...
   public void atualizaIdioma(ResourceBundle b)
   {
      ...
      if(titulo==2)setTitle(bundle.getString("frameTituloConsultar"));
      else setTitle(bundle.getString("frameTituloCadastrar"));
   ...
   private boolean verificarCamposVazios()
   {
      if( tfNome.getText().equals("") ||
          ftfDataNascimento.getText().equals("//") ||
          ftfTelefone.getText().equals("()-") ||  
     ...
     else if( tipoUsuario == 0 && (ftfHorarioDeAcessoInicial.getText().equals(":") ||
                                   ftfHorarioDeAcessoFinal.getText().equals(":")
                                  )  
            )
     {       
         return true;
